...
for command in npm tar mktemp; do
...
expand_npm_package() {
    ...
    archive_name=$(npm pack "$package@$version" --pack-destination "$pack_directory" --silent)
    if [[ -z "$archive_name" || ! -f "$pack_directory/$archive_name" ]]; then
        echo "npm pack failed for $package@$version" >&2
        return 1
    fi
...
monaco_package=$(expand_npm_package 'monaco-editor' "$monaco_version")
...
marked_package=$(expand_npm_package 'marked' "$marked_version")
...
dompurify_package=$(expand_npm_package 'dompurify' "$dompurify_version")
...
codicons_package=$(expand_npm_package '@vscode/codicons' "$codicons_version")
